...
const getHousekeepingDetails = async (req, res) => {
  try {
    const housekeepingDetails = await HousekeepingModel.find().populate(
      "room assignedTo"
    );
...
const getHousekeepingById = async (req, res) => {
  ...
  try {
    const housekeepingDetail = await HousekeepingModel.findById(id).populate(
      "room assignedTo"
    );
...
const createHousekeeping = async (req, res) => {
  ...
  if (!room || !assignedTo || !task) {
    return res
      .status(400)
      .json({ error: "Room, AssignedTo, and Task are required" });
  }
...
const updateHousekeeping = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({
        success: "Housekeeping task updated successfully",
